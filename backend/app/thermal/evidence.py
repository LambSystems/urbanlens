...
def extract_hotspot_thermal(hotspot_centroid: dict, thermal_data: dict) -> dict:
    ...
    for region in hotspot_regions:
        ...
        dist = abs(centroid["lat"] - hotspot_centroid["lat"]) + abs(centroid["lng"] - hotspot_centroid["lng"])
        if dist < best_dist:
            best_dist = dist
            best_match = region
    if best_match is None:
        return {"intensity": 0.0, "estimated_temp_c": mean_temp, "relative_heat": 0.0, "area_px": 0}
