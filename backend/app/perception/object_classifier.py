...
def classify_object(
    ...
    if features["green_ratio"] > 0.34 and features["mean_g"] > features["mean_r"] + 8:
        ...
        hotspot_type = fallback_type
        surface_family = (
            SurfaceFamily.built_surface if fallback_type == HotspotType.roof
            else SurfaceFamily.paved_surface if fallback_type in {HotspotType.parking_lot, HotspotType.road_pavement}
            else SurfaceFamily.mechanical_feature if fallback_type == HotspotType.hvac_mechanical
            else SurfaceFamily.vegetated_area if fallback_type == HotspotType.vegetation_loss
    ...
    if type_confidence < 0.68:
        hotspot_type = HotspotType.other
        label = surface_family.value

    return {
        "hotspot_type": hotspot_type,
        "surface_family": surface_family,
        "type_confidence": type_confidence,
        "object_label": object_label if llm_result else label,
